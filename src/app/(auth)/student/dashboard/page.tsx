...
import { BookOpen, Award, Clock, Star, PlayCircle, User, TrendingUp } from "lucide-react"
...
export default async function StudentDashboard() {
  ...
  const reviews = await Review.find({ student: student._id }).populate("course", "name").lean()
  ...
  const averageRating =
    reviews.length > 0
      ? reviews.reduce((sum: any, review: any) => sum + (typeof review.rating === "number" ? review.rating : 0), 0) /
        reviews.length
      : 0
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              {totalCourses > 0
                ? `${totalCourses === 0 ? 0 : ((completedCourses / totalCourses) * 100).toFixed(0)}% completion rate`
        ...
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-muted-foreground mt-1">Estimated total hours</p>
          </CardContent>
        </Card>
      </div>
              ...
              return (
                <Card key={course._id.toString()} className="overflow-hidden hover:shadow-md transition-shadow">
                  ...
                  <CardHeader className="pb-3">
                    ...
                    <CardDescription>
                      By {course.teacher?.name || "Unknown"} • {course.duration || "Self-paced"}
                    </CardDescription>
                  </CardHeader>
