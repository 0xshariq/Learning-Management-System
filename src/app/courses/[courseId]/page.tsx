...
async function getCourseDetails(courseId: string): Promise<CourseDetails | null> {
  ...
  try {
    ...
    const videos = await VideoModel.find({ course: courseId }).sort({ position: 1 }).lean<any[]>()
    ...
    return {
      ...
      videos: videos.map((video: any): VideoData => ({
...
async function getCourseReviews(courseId: string): Promise<ReviewData[]> {
  ...
  try {
    const reviews = await Review.find({ course: courseId }).populate("student", "name").sort({ createdAt: -1 }).lean<any[]>()

    return reviews.map((review: any): ReviewData => ({
