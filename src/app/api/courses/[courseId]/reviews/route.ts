...
export async function POST(request: NextRequest) {
  try {
    ...
    if (session.user.role !== "student") {
      return NextResponse.json({ message: "Only students can leave reviews for courses." }, { status: 403 })
    }
    ...
    if (!comment || typeof comment !== "string" || comment.trim().length === 0) {
      return NextResponse.json({ message: "Review comment is required and cannot be empty." }, { status: 400 })
    }
    ...
    if (comment.trim().length > 1000) {
      return NextResponse.json({ message: "Review comment must be less than 1000 characters." }, { status: 400 })
    }
    ...
    if (!course) {
      return NextResponse.json({ message: "Course not found. Unable to submit review." }, { status: 404 })
    }
...
// Fix: Only accept (request: NextRequest) and get courseId from query or path
export async function GET(request: NextRequest) {
  try {
    ...
    const formattedReviews: FormattedReview[] = (reviews as PopulatedReview[]).map((review) => ({
      ...
      student: {
        _id: typeof review.student._id === "string" ? review.student._id : review.student._id?.toString?.() || "",
